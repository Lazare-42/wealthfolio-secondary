...
impl ReconciliationService {
    ...
    /// Check if a file hash was already processed by looking at existing ImportRuns.
    fn is_file_already_processed(&self, file_hash: &str) -> Result<bool> {
        // We can't query by source_system directly via trait, so we check
        // by looking at existing runs. We store the hash in checkpoint_out.
        // For efficiency, we'll use a simple approach: the scan stores hashes
        // and we check here.
        //
        // Since ImportRunRepositoryTrait doesn't have a query-by-source-system method,
        // we'll check by getting all NEEDS_REVIEW + APPLIED runs and checking checkpoint_out.
        // This is fine for reasonable numbers of reconciliation runs.
        //
        // Actually — we can't do this efficiently without a repo method. Instead,
        // we'll track processed hashes via the settings KV as a JSON array.
    ...
    fn reconstruct_reconciliation(&self, run: &ImportRun) -> Result<ReconciliationResult> {
        // Read draft activity IDs stored during scan
        ...
        // Reconstruct items from the stored activities
...
impl ReconciliationServiceTrait for ReconciliationService {
    async fn scan_directory(&self) -> Result<ScanResult> {
        ...
        for entry in entries {
            ...
            // Store file metadata and draft activity IDs in checkpoint_out
    ...
    async fn get_pending_reconciliations(&self) -> Result<Vec<ReconciliationResult>> {
        // Get all NEEDS_REVIEW import runs, filter to STATEMENT source_system
        // We'll check recent runs across all accounts
        // The trait doesn't expose get_needs_review, so we use get_recent with a large limit
        // Actually the trait only has get_recent_for_account. We need to iterate or add a method.
        // For simplicity: get config mappings, iterate accounts.
        let config = self.effective_config()?;
    ...
    async fn resolve(&self, request: ResolveRequest) -> Result<ImportRun> {
        ...
        // Accept: promote Draft → Posted by deleting draft and re-creating as posted
        // Actually, we can use get_activity + update_activity
        for id in &request.accept_ids {
            ...
            // Update status to Posted via ActivityUpdate
...
fn parse_amount(s: &str) -> Option<Decimal> {
    // Remove currency symbols, whitespace, thousands separators
...
fn glob_match(pattern: &str, text: &str) -> bool {
    ...
    // If pattern doesn't end with *, the text must end here
