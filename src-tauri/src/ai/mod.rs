...
impl AIProviderConfig {
    ...
    pub fn create_client(&self) -> Result<Option<(AiClient, String)>, String> {
        match self {
            AIProviderConfig::OpenRouter { model } => {
                ...
                let api_key = match api_key {
                    Some(k) => k,
                    None => {
                        return Err("OpenRouter API key not found in env or keyring".to_string())
                    }
                };
            ...
            AIProviderConfig::OpenAI { model } => {
                ...
                let api_key = match api_key {
                    Some(k) => k,
                    None => return Err("OpenAI API key not found in env or keyring".to_string()),
                };
            ...
            AIProviderConfig::Anthropic { model } => {
                ...
                let api_key = match api_key {
                    Some(k) => k,
                    None => return Err("Anthropic API key not found in env or keyring".to_string()),
                };
            ...
            AIProviderConfig::Mistral { model } => {
                ...
                let api_key = match api_key {
                    Some(k) => k,
                    None => return Err("Mistral API key not found in env or keyring".to_string()),
                };
